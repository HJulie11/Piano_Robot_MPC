...
def main(_) -> None:
    ...
    # print(env.task.root_entity.mjcf_model)
    # return

    # model = env.physics.model._model
    # print(env.physics.model.body("rh_shadow_hand/rh_thbase").id)
    # print(env.physics.data.xpos[env.physics.model.body("rh_shadow_hand/rh_thbase").id])
    # return
    
    # Recompile the environment with new bodies
    # print(env._physics)
    ...
    fingertips = ["rh_shadow_hand/thdistal_site", "rh_shadow_hand/ffdistal_site", "rh_shadow_hand/mfdistal_site", "rh_shadow_hand/rfdistal_site", "rh_shadow_hand/lfdistal_site", "rh_shadow_hand/thdistal_site"]
    fingertip_task = []
    ...
    data = env._physics.data._data

    target_ids = []
    target_names = []
    for i in range(len(fingertip_task)):
        fingertip_site = fingertips[i]
        ...
        target_ids.append(target_mocap_id)
        target_names.append(target_mocap_name)

    key_ids = [63, 65, 67, 68, 70, 72]
    key_names = []
    ...
    # key_pos = data.site_xpos[key_idx].copy()
    ...
    with mujoco.viewer.launch_passive(
        ...
        # mink.move_mocap_to_frame(model, data, "th_target", "rh_shadow_hand/thdistal_site", "site")

        t = 0.0

        while viewer.is_running():
            # data.mocap_pos[base_mid] = 
            # base_task.set_target(mink.SE3.from_mocap_name(model, data, "base_target"))
            for i, finger in enumerate(fingertip_task):

                transform_fingertip_target_to_world = (
                    configuration.get_transform_frame_to_world(f"piano/{key_names[i]}", "site")
                )
                key_translation = transform_fingertip_target_to_world.translation()
                finger.set_target(transform_fingertip_target_to_world)
                target_mid = target_ids[i]
                mink.move_mocap_to_frame(model, data, target_names[i], fingertips[i], "site")
                data.mocap_pos[target_mid] = key_translation
                # data.xpos[mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_BODY, "piano/white_key_63")]
                data.mocap_pos[target_mid][2] += 0.02
                data.mocap_pos[target_mid][0] += 0.05
                # fingertip_task.set_target(mink.SE3.from_mocap_name(model, data, "th_target"))
                finger.set_target(mink.SE3.from_mocap_id(data, target_mid))
