...
def main(_) -> None:
    ...
    FINGER_JOINT=[
        ['rh_shadow_hand/rh_THJ5','rh_shadow_hand/rh_THJ4','rh_shadow_hand/rh_THJ3','rh_shadow_hand/rh_THJ2'],
    ...
    key_numbers = [63]#, 65, 67, 68] #, 70]
    fingertips = env.task._hand.fingertip_sites
    qpos = np.zeros(23)
    qpos_hand = np.zeros(23)
    ik_sequence = []

    for i, key_number in enumerate(key_numbers):
        print(f"IK for key {key_number} with fingertip {fingertips[i].name}")
        key_pos = get_key_pos(env, key_number, fingertips[i].name)
        # if "th" in fingertips[i].name:
        #     ikresult=qpos_from_site_pose(env.physics,'rh_shadow_hand/'+ fingertips[i].name, key_pos, None, None, max_steps=500)
        # else:
        ikresult=qpos_from_site_pose(env.physics,'rh_shadow_hand/'+ fingertips[i].name, key_pos, None, FINGER_JOINT[i] + WRIST + forearm, max_steps=500)
        
        qpos_hand[-3:-1] = ikresult.qpos[HAND_INDS]
        qpos[:-1] = ikresult.qpos[QPOS_INDS]
        for _ in range(10):
            ik_sequence.append(qpos_hand) # hover
        
        for _ in range(10):
            ik_sequence.append(qpos) # press

        # for _ in range(5):
        #     ik_sequence.append(qpos_hand) # hover
    print(len(ik_sequence))
    # return 
